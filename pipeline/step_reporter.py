...
def _collect_step5() -> dict:
    ...
    if treefile is None:
        s3_paths = [
            ...
            # Phenotype-namespaced path — populated by whichever phenotype was last run.
            # The generic path above is preferred; this is a legacy fallback.
            "s3://bioresilient-data/step_cache/species.treefile",
        ]
...
def _collect_step14() -> dict:
    ...
    with get_session() as s:
        ...
        # CRISPRGeneDependency.csv uses 0-1 probability scale (>0.7 = broadly essential)
        data["depmap_essential"] = (
            s.query(func.count(SafetyFlag.id))
            .filter(SafetyFlag.depmap_score != None, SafetyFlag.depmap_score > 0.7)
            .scalar() or 0
        )
    return data
...
def _render_md(step: str, data: dict, vr: ValidationResult) -> str:
    ...
    if step == "step1":
        ...
        lines.append(f"- DepMap essential (probability > 0.7): {data.get('depmap_essential', 0):,}")
