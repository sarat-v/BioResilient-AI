...
dropdb bioresilient_test 2>/dev/null || true
createdb bioresilient_test
echo "✓ Database created: bioresilient_test"

# Step 4: Run migrations
echo "[4/6] Running database migrations..."
cd "$REPO_ROOT"
export PATH="/opt/homebrew/opt/postgresql@15/bin:$PATH"
export DATABASE_URL="postgresql://localhost/bioresilient_test"
...
os.environ['DATABASE_URL'] = 'postgresql://localhost/bioresilient_test'
...
echo "  cd $REPO_ROOT"
echo "  DATABASE_URL='postgresql://localhost/bioresilient_test' python3 pipeline/orchestrator.py"
echo ""
echo "Option B: Run API only (to test UI)"
echo "  DATABASE_URL='postgresql://localhost/bioresilient_test' uvicorn api.main:app --host 127.0.0.1 --port 8000"
echo ""
