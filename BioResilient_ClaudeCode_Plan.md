...
```bash
conda create -n bioresilient python=3.11
...
docker build -t bioresilient:latest ./docker/
```
...
### Output
```
s3://bioresilient-data/species/{species_name}/
...
### Output
```
s3://bioresilient-data/processed/{species_name}/
...
### Output
```
s3://bioresilient-data/orthogroups/
...
4. Copy per-orthogroup FASTAs to s3://bioresilient-data/orthogroups/{OG_id}.faa
...
### Output
`s3://bioresilient-data/aligned/OG0000001.aligned.faa` — one per orthogroup (~20,000 files)
...
4. Upload to s3://bioresilient-data/aligned/
...
### Output
`s3://bioresilient-data/codon_aligned/OG0000001.codon.fna`
...
5. Upload to s3://bioresilient-data/codon_aligned/
...
### Output
`s3://bioresilient-data/trimmed/OG0000001.trimmed.fna`
...
- Upload trimmed files to s3://bioresilient-data/trimmed/
```
...
profiles {
    ...
    aws {
        ...
        process.container = '123456789.dkr.ecr.us-east-1.amazonaws.com/bioresilient:latest'
        aws.region = 'us-east-1'
        workDir = 's3://bioresilient-work/nextflow-tmp'
    ...
    gcp {
        ...
        workDir = 'gs://bioresilient-work/nextflow-tmp'
    }
}
```
...
- Use SQLite instead of PostgreSQL (`sqlite:///bioresilient_test.db`)
